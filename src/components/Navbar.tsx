...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <header id="main-header" className="sticky top-0 z-40 w-full bg-white/95 dark:bg-[#101010]/95 backdrop-blur-xl border-b border-slate-200/90 dark:border-white/5 text-slate-900 dark:text-zinc-100 transition-colors duration-300 shadow-xs">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 shrink-0" id="header-actions">
            ...
            <div className="relative hidden sm:block">
              <button
                ...
                className="py-1.5 px-2.5 rounded-xl bg-slate-100 dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/10 border border-slate-200 dark:border-white/10 text-xs font-bold flex items-center gap-1.5 transition-all text-slate-800 dark:text-zinc-200 cursor-pointer"
                title="Select Wholesale Currency"
              >
            ...
            {currentUser ? (
              <div className={`relative ${isProfileMenuOpen ? 'z-50' : ''}`}>
                <button
                  ...
                  className={`relative rounded-full p-0.5 focus:outline-none focus:ring-2 focus:ring-[#F27D26] transition-transform duration-200 hover:scale-105 active:scale-95 cursor-pointer shrink-0 ${
                    isProfileMenuOpen ? 'z-50 ring-2 ring-[#F27D26]' : ''
                  }`}
                  title={`Account Menu: ${userProfile?.displayName || currentUser.email}`}
            ...
            >
              {isMobileMenuOpen ? (
                <X className="w-4 h-4 text-[#F27D26]" />
              ) : (
          ...
          <div className="relative w-full">
            ...
            {searchQuery && (
              <button
                ...
                className="absolute right-2.5 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-700 dark:text-zinc-400 dark:hover:text-white p-1"
                aria-label="Clear search"
              >
                <X className="w-3.5 h-3.5" />
              </button>
